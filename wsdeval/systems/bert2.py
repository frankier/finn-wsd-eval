...
def train(inf, keyin, model, synsets):
    ...
    if synsets:
        manager = VecStorage(model, BERT_SIZE, "wd")
        train_synset_examples(manager, training_examples)
    else:
        manager = VecStorage(model, BERT_SIZE, "wd", value_bytes=10)
...
def test(model, inf, keyout, use_freq, synsets):
    ...
    if synsets:
        manager = VecStorage(model, BERT_SIZE, "rd")
        test_synset_experts(manager, inst_it, keyout, allow_most_freq=use_freq)
    else:
        manager = VecStorage(model, BERT_SIZE, "rd", value_bytes=10)
